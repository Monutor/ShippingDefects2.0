...
async function loadPalletsForTab() {
  ...
  try {
    await palletStore.loadActivePallet()
    if (
    ...
    pallets.value =
      palletStore.pallets?.length > 0
        ? palletStore.pallets.map((p) => ({ ...p, seal: p.seal || null }))
        : []
  ...
  if (allUnloaded.length > 0) {
    ...
    await Promise.all(
      allUnloaded.map(async (box) => {
        ...
        return fromStore && !fromStore.itemsLoaded
          ? boxesStore.refreshBoxItems(box.id)
          : palletStore.refreshAvailableBoxItems(box.id)
      })
    )
  }
    ...
    <div class="w-full mt-2">
      ...
      >
        ...
        <template v-if="activeTab === 0">
              ...
              >
                <div class="flex items-center gap-4 mb-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm text-white/80">📦 {{ viewedBoxItems.length }} товаров</p>
                  </div>
                </div>
                <div
                ...
                >
                  ...
                  >
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <p v-if="item.article" class="text-xs text-white/50 mt-1">
                        {{ item.article }}
                      </p>
                    </div>
                  </div>
                ...
                <div class="mt-4">
                  <Button
                    variant="success"
                    block
                    :disabled="viewedBoxItems.length === 0"
          ...
          <template v-else>
            <BoxesTab
              :boxes="boxesStore.boxes"
              :is-loading="isLoadingPallets"
              @request-delete="requestDeleteBoxWithBox"
              @export-all="exportAllBoxes"
            />
          </template>
        </template>
        ...
        <PalletsTab
          ...
          :is-loading="isLoadingPallets"
          @load-box-items="loadAvailableBoxItems"
          @export-all="exportAllPallets"
        />
    ...
    >
      <div v-if="selectedBox" class="box-modal-content">
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="box-title-section">
              ...
              <p class="box-subtitle text-white/70 text-sm">
                {{
                  new Date(selectedBox.createdAt).toLocaleString('ru-RU', {
                    day: 'numeric',
                    month: 'numeric',
                    year: 'numeric',
                    hour: '2-digit',
        ...
        <div class="items-section mb-4">
          ...
          >
            <van-icon name="bag-o" class="text-primary-400" aria-hidden="true" /> Содержимое
          </div>
        ...
        <Button block class="custom-btn-primary" @click="exportBox(selectedBox)"
          ><van-icon name="down" class="" aria-hidden="true" /> Скачать Excel</Button
        >
      </div>
    </Modal>
