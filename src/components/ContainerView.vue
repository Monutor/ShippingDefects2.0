...
import ScannerSection from '@/components/ScannerSection.vue'
...
function handleScanModeChange(mode) {
  // scanner — реактивный объект из useScanner, мутации допустимы
...
function handleSelectCamera(deviceId) {
  // scanner — реактивный объект из useScanner, мутации допустимы
  // eslint-disable-next-line vue/no-mutating-props
  props.scanner.selectedCameraId.value = deviceId
}

function handleTsdChange(val) {
  // eslint-disable-next-line vue/no-mutating-props
  props.scanner.tsdInput.value = val
}
    ...
    <main v-show="!isLoading" class="flex-1 flex flex-col p-4 gap-4 overflow-y-auto">
      ...
      >
        <div class="flex items-center justify-center gap-3 mb-2">
          <img :src="containerIconUrl()" class="w-6 h-6 opacity-80" :alt="containerLabel" />
          ...
          >
            <div class="flex items-center">
              <span class="text-sm font-medium text-slate-100 flex items-center gap-2">
                <img :src="containerIconUrl()" class="w-5 h-5 opacity-60" :alt="containerLabel" />
      ...
      <div v-if="currentContainer" class="bg-slate-800/80 border border-slate-700 rounded-2xl p-4">
        ...
        <ScannerSection
          :scanner="scanner"
          :on-tsd-submit="onTsdSubmit"
          @mode-change="handleScanModeChange"
          @select-camera="handleSelectCamera"
          @tsd-change="handleTsdChange"
        />
        <template v-if="scanner.scanMode.value === 'camera'">
      ...
      <div v-if="currentContainer && showMixesSection" class="mt-4">
      ...
      <!-- Action Buttons -->
      <div v-if="currentContainer" class="box-actions space-y-2">
    ...
    >
      ...
      <div class="scanner-element rounded-2xl mb-4 relative">
        ...
        <button
          ...
          type="button"
          @click="scanner.toggleFlashlight()"
        >
          <svg
            width="28"
            height="28"
