...
import {
  syncBrainItems,
  loadBrainItemsFromBackend,
  deleteBrainItemsFromBackend
} from '@/utils/sync'

/**
 * Store для управления базой бракованных товаров
 * Данные загружаются из Excel и хранятся в IndexedDB (локально)
 */
export const useBrainStore = defineStore(
  'brain',
  () => {
    ...
    async function setDatabase(data, mapping) {
      ...
      // Синхронизация с локальной базой (в фоне, не блокирует UI)
      isSyncing.value = true
      syncError.value = null
      try {
        await syncBrainItems(items.value, mapping)
      } catch {
        syncError.value = 'Не удалось сохранить данные в локальное хранилище.'
      } finally {
    ...
    /**
     * Загрузка базы из локального хранилища при инициализации
     */
    async function loadFromBackend() {
      ...
      try {
        ...
        loadError.value = 'Не удалось загрузить данные. Используем локальные данные.'
      } finally {
    ...
    /**
     * Очистка базы данных (single-user — локальная очистка с подтверждением)
     */
  ...
  {
    // Локальное хранение в IndexedDB — бизнес-данные не персистятся в localStorage
  }
)
