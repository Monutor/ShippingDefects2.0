...
export function usePalletLoading(context) {
    ...
    try {
      ...
      pallets.value = (result.data || [])
        ...
        .map((p) => ({
          ...
          // finished_at пишется с этого фикса; у старых завершённых паллет
          // его нет — падаем на updated_at (≈ момент завершения) или created_at,
          // чтобы не показывать «Invalid Date».
          finishedAt: p.finished_at || p.updated_at || p.created_at,
          items: []
        }))
  ...
  async function loadActivePallet() {
    try {
      ...
      // Single-user: сборщик один — берём самый новый активный паллет
      const sorted = [...activePallets].sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      )
      const myPallet = sorted[0] || null
  ...
  async function loadActivePalletById(pallet) {
    let serverItems = []
    try {
      const itemsResult = await db.palletItems.getByPalletId(pallet.id)
      if (itemsResult?.data) {
        ...
        for (const boxRef of boxItems) {
          try {
            ...
            // Пропускаем микс, который не удалось загрузить, — остальные всё равно попадут в паллет
          }
        }
            ...
            if (itemsResult?.data) {
              ...
              if (currentPallet.value && currentPallet.value.id === pallet.id) {
                ...
                if (localItemCount > 0 && localItemCount >= serverItemCount) {
                  // Локальных товаров не меньше — оставляем локальное состояние
                } else if (currentPallet.value.id === pallet.id && localItemCount === 0) {
                  currentPallet.value.items = serverItems
                } else {
                  // Во всех остальных случаях тоже оставляем как есть
                }
              }
            }
          } catch (err) {
