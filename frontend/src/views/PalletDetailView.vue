...
<template>
  <div class="pallet-detail-view min-h-screen bg-slate-900 pb-16">
    <!-- Nav Bar -->
    <NavBar title="Паллет" left-text="Назад" left-arrow @click-left="$router.back()" />
    ...
    <template v-else-if="pallet">
      <div class="p-4 flex-1 flex flex-col min-h-0">
        ...
        >
          <!-- Бейдж владельца -->
          <span
            v-if="pallet.collector_id && !isOwner(pallet.collector_id)"
          ...
          <div
            ...
            :class="{
              'opacity-60 pointer-events-none': pallet.collector_id && !isOwner(pallet.collector_id)
            }"
          >
            <div
          ...
          <div class="flex items-center gap-4 text-sm">
            ...
            <span
              >📦 Позиций: <strong class="text-slate-200">{{ palletItems.length }}</strong></span
            >
          </div>
        ...
        <div v-if="palletItems.length > 0" class="flex-1 flex flex-col min-h-0 overflow-hidden">
          <h4 class="text-sm font-medium text-slate-300 mb-2 shrink-0">
            Содержимое ({{ palletItems.length }} шт.)
          </h4>
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p v-if="item.isBoxItem" class="text-xs text-indigo-400 mt-1">
                    📦 {{ item.boxName }}
                  </p>
                </div>
              </div>
