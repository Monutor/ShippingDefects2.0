...
<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <!-- Default -->
        <div v-if="type === 'default'" class="bg-slate-800/95 backdrop-blur-md border border-slate-700 rounded-2xl px-6 py-4 shadow-2xl">
          <p class="text-slate-100 text-sm font-medium whitespace-nowrap">{{ message }}</p>
        </div>

        <!-- Success -->
        <div v-else-if="type === 'success'" class="bg-emerald-900/95 backdrop-blur-md border border-emerald-600 rounded-2xl px-6 py-4 shadow-2xl shadow-emerald-500/10">
          <p class="text-emerald-100 text-sm font-medium whitespace-nowrap">{{ message }}</p>
        </div>

        <!-- Warning -->
        <div v-else-if="type === 'warning'" class="bg-amber-900/95 backdrop-blur-md border border-amber-600 rounded-2xl px-6 py-4 shadow-2xl shadow-amber-500/10">
          <p class="text-amber-100 text-sm font-medium whitespace-nowrap">{{ message }}</p>
        </div>

        <!-- Error — чужой контейнер -->
        <div v-else-if="type === 'error'" class="bg-red-900/95 backdrop-blur-md border border-red-600 rounded-2xl px-6 py-4 shadow-2xl shadow-red-500/15">
