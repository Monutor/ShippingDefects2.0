...
function handleTsdKeyup(event, processCallback) {
  if (event.key === 'Enter') {
    props.scanner.handleTsdInput(processCallback)
  }
}
...
<template>
  <div class="container-view bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 pb-20">
    ...
    <main v-show="!isLoading" class="flex-1 flex flex-col p-4 gap-4 overflow-y-auto">
      <!-- Status Bar -->
      <div v-if="currentContainer" class="status-bar bg-slate-800/80 border border-slate-700 rounded-2xl p-4 text-center">
        <div class="flex items-center justify-center gap-3 mb-2">
          <Badge :count="containerItemCount" variant="info" />
          <span class="text-slate-100 font-medium">{{ currentContainer?.name || `${containerLabel} не выбран` }}</span>
        </div>
      </div>

      <!-- Empty state: no containers at all -->
      <div v-if="!currentContainer && activeContainers.length === 0" class="absolute inset-0 flex flex-col items-center justify-center">
        <button class="w-20 h-20 rounded-full bg-blue-600 hover:bg-blue-500 active:bg-blue-700 flex items-center justify-center transition-colors shadow-lg shadow-blue-600/30" @click="$emit('create-container')">
      ...
      <div v-if="!currentContainer && activeContainers.length > 0" class="flex flex-col items-center justify-center py-8">
        <button class="w-16 h-16 rounded-full bg-blue-600 hover:bg-blue-500 active:bg-blue-700 flex items-center justify-center transition-colors shadow-lg shadow-blue-600/30" @click="$emit('create-container')">
      ...
      <div v-if="!currentContainer && activeContainers.length > 0" class="bg-slate-800/80 border border-slate-700 rounded-2xl p-4">
        ...
        <div class="space-y-2 max-h-64 overflow-y-auto scrollbar-thin">
          <div v-for="container in activeContainers" :key="'c' + container.id" class="item-row p-3 rounded-xl bg-slate-700/80 hover:bg-slate-600 cursor-pointer transition-colors" @click="$emit('select-container', container)">
            <div class="flex items-center justify-between">
              <span class="text-sm font-medium text-slate-100">{{ container.name || containerLabel }}</span>
      ...
      <div v-if="currentContainer" class="bg-slate-800/80 border border-slate-700 rounded-2xl p-4">
        ...
        <div class="mode-switcher mb-4">
          <div class="grid grid-cols-2 gap-3">
            <button :class="['py-3.5 rounded-xl text-base font-semibold transition-all duration-200 border-none cursor-pointer flex items-center justify-center gap-2', scanner.scanMode.value === 'tsd' ? 'bg-gradient-to-r from-primary-500 to-primary-700 text-white shadow-lg shadow-primary-500/30' : 'bg-slate-800/80 text-slate-400 border border-slate-700 hover:bg-slate-700']" @click="handleScanModeChange('tsd')">
              <img src="/img/bank-terminal.svg" alt="ТСД" class="w-5 h-5" /> ТСД
            </button>
            <button :class="['py-3.5 rounded-xl text-base font-semibold transition-all duration-200 border-none cursor-pointer flex items-center justify-center gap-2', scanner.scanMode.value === 'camera' ? 'bg-gradient-to-r from-purple-500 to-purple-700 text-white shadow-lg shadow-purple-500/30' : 'bg-slate-800/80 text-slate-400 border border-slate-700 hover:bg-slate-700']" @click="handleScanModeChange('camera')">
        ...
        <div v-if="scanner.scanMode.value === 'tsd'" class="space-y-3 mt-4">
          <Input v-model="scanner.tsdInput.value" placeholder="📱 Введите штрихкод и нажмите Enter" :disabled="scanner.isProcessingTsd.value" variant="primary" size="large" @keyup.enter="handleTsdKeyup($event, $emit('scan', $event))" />
        ...
        <div v-if="scanner.scanMode.value === 'camera'" class="space-y-3">
          <Button variant="primary" block @click="handleScannerToggle">
            <van-icon :name="scanner.isScanning.value ? 'stop-circle-o' : 'scan'" size="20" aria-hidden="true" />
      ...
      <div v-if="currentContainer && isOwner && showMixesSection" class="mt-4">
        <div class="bg-slate-800/80 border border-slate-700 rounded-2xl overflow-hidden">
          <div class="p-4 border-b border-slate-700 flex items-center justify-between">
            <h3 class="font-semibold text-slate-100 flex items-center gap-2"><van-icon name="bag" class="text-primary-400" aria-hidden="true" /> Готовые миксы</h3>
            <button class="text-primary-400 text-sm flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-slate-700 transition-colors" aria-label="Обновить список миксов" @click="$emit('load-mixes')"><van-icon name="replay" size="14" aria-hidden="true" /> Обновить</button>
          </div>
          <div class="p-4">
            ...
            <div v-else class="space-y-2 max-h-64 overflow-y-auto">
              <div v-for="mix in availableMixes" :key="mix.id" class="flex items-center justify-between p-3 bg-slate-700/80 rounded-xl">
                ...
                <button class="ml-3 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold rounded-lg transition-colors flex-shrink-0" @click="$emit('add-mix', mix)">+ Добавить</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Items List -->
      <div v-if="currentContainer" class="bg-slate-800/80 border border-slate-700 rounded-2xl overflow-hidden">
        <div class="p-4 border-b border-slate-700 flex items-center justify-between">
          <h3 class="font-semibold text-slate-100 flex items-center gap-2"><van-icon name="bag-o" class="text-primary-400" aria-hidden="true" /> Товары в {{ containerLabel }}е ({{ containerItemCount }})</h3>
          <button v-if="showMixesSection" class="text-primary-400 text-sm flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-slate-700 transition-colors" aria-label="Обновить список товаров" @click="$emit('refresh-items')"><van-icon name="replay" size="14" aria-hidden="true" /> Обновить</button>
        </div>
        <div class="p-4">
          ...
          <div v-else class="items-list-detail space-y-2 max-h-72 overflow-y-auto scrollbar-thin">
            <div v-for="(item, index) in items" :key="index" class="item-row p-3 rounded-xl transition-all duration-200 relative" :class="getItemHighlight(item, index)">
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" :class="getItemHighlight(item, index).includes('emerald') ? 'bg-emerald-500/20 border border-emerald-500/30' : 'bg-primary-500/20 border border-primary-500/30'">
                  <span class="text-xs font-bold" :class="getItemHighlight(item, index).includes('emerald') ? 'text-emerald-400' : 'text-primary-400'">{{ index + 1 }}</span>
                </div>
                <div class="flex-1 min-w-0">
                ...
                <button v-if="isOwner" class="w-8 h-8 rounded-lg bg-red-500/20 border border-red-500/30 flex items-center justify-center flex-shrink-0 hover:bg-red-500/40 transition-colors" aria-label="Удалить товар" @click="$emit('remove-item', item)"><van-icon name="delete-o" size="16" color="#ef4444" aria-hidden="true" /></button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="currentContainer && isOwner" class="box-actions space-y-2">
        <div class="grid grid-cols-3 gap-2">
          <Button variant="secondary" :disabled="!canUndo" class="custom-btn-secondary" @click="$emit('undo')">
          ...
          <Button variant="success" :disabled="containerItemCount === 0" :class="containerItemCount === 0 ? 'opacity-50' : ''" class="custom-btn-success" @click="$emit('finish')">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" /></svg> {{ finishButtonText }}
          </Button>
        </div>
      </div>
    </main>
    ...
    <!-- Scanner Modal -->
    <Modal :model-value="showScanner" confirm-text="Стоп" confirm-color="danger" @update:model-value="handleScannerModalUpdate" @confirm="$emit('stop-scanner')">
    ...
    <!-- Stop Item Modal -->
    <Modal :model-value="showStopItemModal" title="⛔ Стоп-товар!" :show-cancel="false" confirm-text="OK" @update:model-value="$emit('update:showStopItemModal', $event)">
    ...
    <!-- Remove Item Modal -->
    <Modal :model-value="showRemoveModal" title="Удалить товар?" show-cancel confirm-text="Удалить" cancel-text="Отмена" confirm-color="danger" @update:model-value="$emit('update:showRemoveModal', $event)" @confirm="$emit('confirm-remove')">
      <p class="text-slate-400 text-center">{{ removeItemRef?.name }} ({{ removeItemRef?.number }})</p>
    </Modal>

    <!-- Finish Modal -->
    <Modal :model-value="showFinishModal" :title="modalTitle" show-cancel :confirm-text="finishModalConfirmText" cancel-text="Отмена" @update:model-value="$emit('update:showFinishModal', $event)" @confirm="$emit('confirm-finish')">
      <div class="text-left space-y-3 text-white">
        <p>{{ modalDescription }}</p>
        <div class="bg-slate-800/50 rounded-lg p-4">
          <p><strong>{{ containerType === 'pallet' ? 'Паллет' : 'Микс' }}:</strong> {{ currentContainer?.name }}</p>
...
.container-view { min-height: 100vh; }
main { flex: 1; }
