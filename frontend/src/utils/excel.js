...
export async function exportBoxToExcel(box, collector = {}) {
    ...
    try {
        ...
        dateRow.getCell('B').value = new Date(box.createdAt || box.created_at || Date.now()).toLocaleString('ru-RU', {
            day: 'numeric',
            month: 'numeric',
            year: 'numeric',
            hour: '2-digit',
...
export async function exportPalletToExcel(pallet, collector = {}) {
    ...
    try {
        ...
        if (pallet.items && pallet.items.length > 0) {
            for (const localItem of pallet.items) {
                if (localItem.source_type !== 'box') {
                    const idx = palletItems.findIndex(i => i.source_type === localItem.source_type && i.source_id === localItem.source_id)
                    if (idx !== -1) {
        ...
        for (const item of palletItems) {
            ...
            if (item.source_type === 'box') {
                ...
                if (boxItems) {
                    for (const boxItem of boxItems) {
                        ...
                        const boxScannedAt = boxItem.scanned_at || boxItem.data_scanned_at || boxItem.scannedAt || ''
                        contentSheet.addRow({ type: '', identifier: boxItem.barcode || boxItem.number || '', name: boxItem.name || '', article: boxItem.brand || boxItem.article || '', comment: boxItem.comment || '', scannedAt: boxScannedAt ? new Date(boxScannedAt).toLocaleString('ru-RU') : '' })
                    }
                }
                ...
                const sepScannedAt = data.scanned_at || data.data_scanned_at || data.scannedAt || item.scannedAt || ''
                contentSheet.addRow({
                    type: '📋 Паллет',
            ...
            } else if (item.source_type === 'inline' || item.source_type === 'pallet') {  // inline/pallet items паллета — данные из pallet_items напрямую
                const identifier = item.item_barcode || item.source_id || ''

                // Читаем поля напрямую из pallet_items — без _full_data обёртки
                const name = (item.item_name && item.item_name.trim()) ? item.item_name : `Товар ${identifier}`
        ...
        contentSheet.eachRow((row, rowNumber) => { if (rowNumber > 1) row.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } } })

        // === Лист 2: Пломба ===
        const sealSheet = workbook.addWorksheet('Пломба')
        sealSheet.columns = [{ key: 'a', width: 35 }, { key: 'b', width: 35 }]

        const blueColor = { argb: 'FF1989FA' }
        const blackColor = { argb: 'FF000000' }
        const grayColor = { argb: 'FF999999' }

        sealSheet.getCell('A1').value = 'ПЛОМБА'; sealSheet.getCell('A1').font = { bold: true, size: 20, color: blueColor }; sealSheet.getCell('A1').alignment = { horizontal: 'center', vertical: 'middle' }; sealSheet.mergeCells('A1:B1'); sealSheet.getRow(1).height = 35
        sealSheet.getCell('A2').value = 'ОП № 187 г.Тамбов'; sealSheet.getCell('A2').font = { bold: true, size: 16, color: blueColor }; sealSheet.getCell('A2').alignment = { horizontal: 'center', vertical: 'middle' }; sealSheet.mergeCells('A2:B2'); sealSheet.getRow(2).height = 30

        const palletCell = sealSheet.getCell('A3')
        palletCell.value = `Паллет №${pallet.number}`; palletCell.font = { bold: true, size: 14, color: blackColor }; palletCell.alignment = { horizontal: 'center', vertical: 'middle' }
        sealSheet.mergeCells('A3:B3'); sealSheet.getRow(3).height = 28

        let totalBoxes = 0, totalItems = 0
        // Используем deduplicated palletItems вместо raw pallet.items — защита от дубликатов
        const itemsList = palletItems || []
        itemsList.forEach(i => { if (i.source_type === 'box') totalBoxes++ })
        // Считаем separate_item + inline как отдельные товары
        totalItems = itemsList.filter(i => i.source_type === 'separate_item' || i.source_type === 'inline').length

        sealSheet.getRow(4).getCell('A').value = 'Коробов:'; sealSheet.getRow(4).getCell('A').font = { bold: true, size: 11 }; sealSheet.getRow(4).getCell('A').alignment = { horizontal: 'left', vertical: 'middle' }; sealSheet.getRow(4).getCell('B').value = `${totalBoxes} шт.`; sealSheet.getRow(4).getCell('B').font = { size: 11 }; sealSheet.getRow(4).getCell('B').alignment = { horizontal: 'right', vertical: 'middle' }; sealSheet.getRow(4).height = 25
        sealSheet.getRow(5).getCell('A').value = 'Товаров в паллете:'; sealSheet.getRow(5).getCell('A').font = { bold: true, size: 11 }; sealSheet.getRow(5).getCell('A').alignment = { horizontal: 'left', vertical: 'middle' }; sealSheet.getRow(5).getCell('B').value = `${totalItems} шт.`; sealSheet.getRow(5).getCell('B').font = { size: 11 }; sealSheet.getRow(5).getCell('B').alignment = { horizontal: 'right', vertical: 'middle' }; sealSheet.getRow(5).height = 25

        const dateRow = sealSheet.getRow(6)
        dateRow.getCell('A').value = 'Дата формирования:'; dateRow.getCell('A').font = { bold: true, size: 11 }; dateRow.getCell('A').alignment = { horizontal: 'left', vertical: 'middle' }
        const dateVal = pallet.createdAt || pallet.created_at || new Date()
        dateRow.getCell('B').value = new Date(dateVal).toLocaleString('ru-RU', { day: 'numeric', month: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' }); dateRow.getCell('B').font = { size: 11 }; dateRow.getCell('B').alignment = { horizontal: 'right', vertical: 'middle' }
        dateRow.height = 25

        sealSheet.getRow(7).getCell('A').value = 'Должность:'; sealSheet.getRow(7).getCell('A').font = { bold: true, size: 11 }; sealSheet.getRow(7).getCell('A').alignment = { horizontal: 'left', vertical: 'middle' }
        sealSheet.getRow(7).getCell('B').value = collector.position || ''; sealSheet.getRow(7).getCell('B').font = { italic: true, size: 11 }; sealSheet.getRow(7).getCell('B').alignment = { horizontal: 'right', vertical: 'middle' }
        sealSheet.getRow(7).height = 25

        sealSheet.getRow(8).getCell('A').value = 'ФИО сборщика:'; sealSheet.getRow(8).getCell('A').font = { bold: true, size: 11 }; sealSheet.getRow(8).getCell('A').alignment = { horizontal: 'left', vertical: 'middle' }
        sealSheet.getRow(8).getCell('B').value = collector.fullName || ''; sealSheet.getRow(8).getCell('B').font = { italic: true, size: 11 }; sealSheet.getRow(8).getCell('B').alignment = { horizontal: 'right', vertical: 'middle' }
        sealSheet.getRow(8).height = 25

        sealSheet.getRow(9).getCell('A').value = 'Подпись:'; sealSheet.getRow(9).getCell('A').font = { bold: true, size: 11 }; sealSheet.getRow(9).getCell('A').alignment = { horizontal: 'left', vertical: 'middle' }; sealSheet.getRow(9).getCell('B').value = ''; sealSheet.getRow(9).height = 30

        sealSheet.getRow(4).eachCell(cell => { cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } } })
        sealSheet.getRow(5).eachCell(cell => { cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } } })
        dateRow.eachCell(cell => { cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } } })
        sealSheet.getRow(7).eachCell(cell => { cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } } })
        sealSheet.getRow(8).eachCell(cell => { cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } } })
        sealSheet.getRow(9).eachCell(cell => { cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } } })

        sealSheet.getCell('A10').value = 'М.П.'; sealSheet.getCell('A10').font = { bold: true, size: 14, color: blackColor }; sealSheet.mergeCells('A10:B10')
        const footerCell = sealSheet.getCell('A11')
        footerCell.value = 'Пломба наклеивается на паллет для защиты содержимого'
        footerCell.font = { italic: true, size: 9, color: grayColor }; footerCell.alignment = { horizontal: 'center', vertical: 'middle' }
        sealSheet.mergeCells('A11:B11')

        for (let i = 1; i <= 11; i++) sealSheet.getRow(i).eachCell(cell => { cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } } })
        ...
        const url = window.URL.createObjectURL(blob)
        const link = document.createElement('a')
        link.href = url; link.download = fullName; document.body.appendChild(link); link.click(); document.body.removeChild(link)
        window.URL.revokeObjectURL(url)
